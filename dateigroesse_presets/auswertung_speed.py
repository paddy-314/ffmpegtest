...
for i, preset in enumerate(unique_presets):
    ...
    ax.bar(index + i * bar_width, preset_data['Speed'], bar_width, label=f'{preset}', color=colors[i])

# Anpassung der Achsenbeschriftungen und des Titels
ax.set_xlabel('CRF-Wert')
ax.set_ylabel('Speed')
ax.set_title('Einfluss der Presets auf Speed nach CRF-Wert')
...
plt.tight_layout()
plt.savefig(os.path.join(work_path, "crf_grouped_bar_chart_speed.pdf"))
